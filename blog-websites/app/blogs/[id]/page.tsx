...
import { User, ArrowLeft, Clock, Link as LinkIcon, Check } from "lucide-react";
...
interface Blog {
  ...
  title: string;
  description: string;
  image: string | StaticImageData; // local blog_data uses imported images, DB uses URL strings
  author: string;
  ...
  useEffect(() => {
    ...
    const fetchBlogData = async () => {
      try {
        ...
        // Check local blog first
        const localBlog = blog_data.find(
          (item) => item.id != null && item.id.toString() === id,
        );

        if (localBlog) {
          if (!ignore) setData(localBlog as unknown as Blog);
          return;
        }
    ...
    return (
      <div className="min-h-screen flex flex-col justify-center items-center bg-[#F4F5F1] px-6 text-center">
        ...
        <p className="mt-3 text-[#14171F]/60 max-w-sm">
          The page you looking for does exist or may have been moved.
        </p>
        <Link
          href="/"
          ...
          <div ref={articleRef}>
            <article className="max-w-none">
              ...
              <p className="text-[17px] leading-[1.75] text-[#14171F]/85">
                ...
                goals. Whether you learning web development, building a startup,
                or improving your skills, consistency and continuous learning
              ...
              <ul className="space-y-3">
                {[
                  ...
                  <li
                    key={item}
                    className="flex items-start gap-3 text-[17px] leading-[1.7] text-[#14171F]/85"
                  >
