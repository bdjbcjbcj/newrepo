...
interface Subscriber {
  _id: string;
  email: string;
  date: string; // renamed from `Date` to avoid shadowing the global Date type
}
...
useEffect(() => {
    ...
    const fetchEmails = async () => {
      try {
        const res = await axios.get<EmailListResponse>("/api/email");
        ...
        console.log(error);
        if (!ignore) toast.error("Failed to load subscriptions");
      } finally {
...
const deleteEmail = async (id: string | number) => {
    try {
      const res = await axios.delete<DeleteResponse>(`/api/email?id=${id}`);

      if (res.data.success) {
        toast.success(res.data.message);

        setEmails((prev) => prev.filter((item) => item._id !== String(id)));
      } else {
