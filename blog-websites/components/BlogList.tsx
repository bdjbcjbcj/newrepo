...
interface Blog {
  ...
  title: string;
  description: string;
  category?: string; // optional now, matches the `item.category &&` guard below
  image?: string;
...
// Cast local static data to the Blog shape once, at the boundary,
// instead of letting it silently widen `allBlogs` to `any[]`.
const typedLocalBlogs: Blog[] = blog_data as unknown as Blog[];
...
const BlogList = () => {
  ...
  >({});
  const [countsMap, setCountsMap] = useState<Record<string | number, number>>(
    {},
  );
  ...
  useEffect(() => {
    ...
    const fetchBlogs = async () => {
      try {
        ...
        if (!ignore) setBlogs(response.data.blogs || []);
      } catch (error) {
  ...
  // Merge local + database blogs (both now guaranteed Blog[])
  const allBlogs = useMemo<Blog[]>(() => {
    return [...typedLocalBlogs, ...blogs];
  }, [blogs]);
            ...
            >
              ...
              <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
                {loadingComments ? (
                  ...
                  (commentsMap[activeCommentId] || []).map((c: Comment) => (
                    <div
                      key={c.id || c._id}
